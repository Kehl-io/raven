...
test("app loads persisted state from the HTTP backend", async ({ page, request }) => {
  ...
  const runResult = await backendCommand<{ run?: { status?: string; blocked_reason?: string; blockedReason?: string }; artifact?: { title?: string } }>(
    request,
    "run_workflow",
    { workflowId: "current-weather" },
  );
  if (!runResult.artifact?.title) {
    const debugInfo = JSON.stringify({
      status: runResult.run?.status,
      blocked: runResult.run?.blocked_reason ?? runResult.run?.blockedReason,
    });
    throw new Error(`run_workflow returned no artifact. Run details: ${debugInfo}`);
  }
  ...
  await expect(
    page.locator(".artifact-list").getByRole("button", { name: /Current Weather/ }),
  ).toBeVisible();
...
test("Create Workflow Hub describes catalog-planned deterministic tools", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Create enabled workflow" })).toBeEnabled();
  await expect(page.getByRole("button", { name: "Create and run once" })).toBeEnabled();
});
